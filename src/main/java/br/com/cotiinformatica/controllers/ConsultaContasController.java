...
public class ConsultaContasController {
	...
	public ModelAndView consultaContasPost(HttpServletRequest request) {
		...
		try {
			...
			Date filtroDataMin = new SimpleDateFormat("yyyy-MM-dd").parse(request.getParameter("dataMin"));
			Date filtroDataMax = new SimpleDateFormat("yyyy-MM-dd").parse(request.getParameter("dataMax"));
	...
	public ModelAndView exclusaoContas(HttpServletRequest request) {
		...
		try {
			...
			if (conta!= null && conta.getIdUsuario()==usuario.getIdUsuario()) {
				...
				modelAndView.addObject("mensagem_sucesso", "apagado com sucesso ");
